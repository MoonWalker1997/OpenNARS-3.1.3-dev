...
public class EventBufferMC {
    ...
    public EventBufferMC(int num_slot, int observation_capacity, int anticipation_capacity, int prediction_capacity, Memory memory) {
        this.memory = memory;
    ...
    // generate the combination with N items in this.concurrent_observation
    ...
    public void C(int M, int N, ArrayList<Task> cpds) {
        ...
        for (i = N; i < M; i++) {
            ...
            if (N > 1)
                ...
                for (j = 0; j < M - 1; j++)
                    tmp.add(this.data.get(this.curr).getConcurrent_observations().get_data().get(b[j]).getContent());
                ...
                for (j = 1; j < tmp.size(); j++) {
                    truth = induction(truth, tmp.get(i).getSentence().getTruth());
                }
...
//        }
        ...
        if (show) {
            System.out.println(this.data.get(this.curr).getHighest_compound().getContent().getName());
        }
